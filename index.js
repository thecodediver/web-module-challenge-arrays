...
function getAverageWordLength(array){
    ...
    for(let i = 0; i < array.length; i++) {
        let splitItem = array[i].split(" ").length;
        wordCount += splitItem;
    }
    return wordCount / arrayLength;
}
...
function getRandomFlavors(originalFlavors, newFlavors, seasonalFlavors, regionalFlavors){
    const newArray = [...originalFlavors, ...newFlavors, ...seasonalFlavors, ...regionalFlavors];
    const finalArray = [];
    for(let i = 0; i < 31; i++) {
        let randomFlavorIndex = Math.floor(Math.random() * newArray.length);
        finalArray.push(newArray.splice(randomFlavorIndex, 1)[0]); 
    }
    return finalArray;
}
